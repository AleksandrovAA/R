...
$$f(X) = 17+0.02*X -0.005*(X-45)^2 + 0.00006*(X-54)^3$$
...
$$X \sim U(5, 105)$$
$$Y = f(X) + \epsilon, где f(X) = 17+0.02*X -0.005*(X-45)^2 + 0.00006*(X-54)^3; \epsilon \sim N(0,1).$$
...
y.func <- function(x) {17 + 0.02*x - 0.005*(x - 45)^2 + 0.00006*(x - 54)^3}
...
Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 10 и равно 1.860475. По графику мы можем установить, что первое значение MSEТЕСТ df = 8. 
...
Наименьшее значение MSE на тестовой выборке df = 8 и равно 1.632344. По графику можно заметить, что первое значение MSEТЕСТ df = 6. 
...
Наименьшее значение MSE на тестовой выборке  df = 10 и равно 1.860475. По графику можно предположить, что значение MSEТЕСТ,  соответствует df = 9.


- Почему при таком изменении данных MSE меняется именно так?

Изначально имеется доволЬно большой процент выборки и при ее уменьшении (доля обучающей выборки меняется от 0.9 до 0.8)  так же имеет значение что модель  пытается принять во внимание все имеющиесяя точки.
